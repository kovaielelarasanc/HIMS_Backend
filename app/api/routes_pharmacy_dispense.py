...
# from app.models.pharmacy import (PharmacyInventoryLot, PharmacyInventoryTxn,
#                                  PharmacySale, PharmacySaleItem,
#                                  PharmacyMedicine, PharmacyPrescriptionItem,
#                                  PharmacyPrescription, PharmacyLocation)
# from app.models.patient import Patient
# from app.schemas.pharmacy import DispenseIn, SaleWithItemsOut, SaleItemOut
...
# from app.services.pdf_pharmacy import build_sale_bill_pdf
...
# @router.get("/sales", response_model=List[SaleWithItemsOut])
# def list_sales(
...
# ):
#     """
#     List pharmacy sales (for Pharmacy Billing screen).
#     """
#     if not has_perm(user, "pharmacy.view"):
#         raise HTTPException(403, "Not permitted")

#     qry = db.query(PharmacySale).order_by(PharmacySale.id.desc())
#     if patient_id:
#         qry = qry.filter(PharmacySale.patient_id == patient_id)
#     if context_type in ("opd", "ipd"):
#         qry = qry.filter(PharmacySale.context_type == context_type)
#     if from_date:
#         start_dt = datetime.combine(from_date, datetime.min.time())
#         qry = qry.filter(PharmacySale.created_at >= start_dt)
#     if to_date:
#         end_dt = datetime.combine(to_date, datetime.max.time())
#         qry = qry.filter(PharmacySale.created_at <= end_dt)

#     sales = qry.limit(limit).all()

#     out: List[SaleWithItemsOut] = []
#     for s in sales:
#         items = (db.query(PharmacySaleItem).filter(
#             PharmacySaleItem.sale_id == s.id).order_by(
#                 PharmacySaleItem.id.asc()).all())
#         sale_out = SaleWithItemsOut.model_validate(s, from_attributes=True)
#         sale_out.items = [
#             SaleItemOut.model_validate(it, from_attributes=True)
#             for it in items
#         ]
#         out.append(sale_out)
#     return out
...
# @router.get("/sales/{sale_id}", response_model=SaleWithItemsOut)
# def get_sale(
#         sale_id: int,
#         db: Session = Depends(get_db),
#         user: User = Depends(auth_current_user),
# ):
#     """
#     Get a single pharmacy sale + its items.
#     """
#     if not has_perm(user, "pharmacy.view"):
#         raise HTTPException(403, "Not permitted")

#     sale = db.query(PharmacySale).get(sale_id)
#     if not sale:
#         raise HTTPException(404, "Sale not found")

#     items = (db.query(PharmacySaleItem).filter(
#         PharmacySaleItem.sale_id == sale_id).order_by(
#             PharmacySaleItem.id.asc()).all())
#     sale_out = SaleWithItemsOut.model_validate(sale, from_attributes=True)
#     sale_out.items = [
#         SaleItemOut.model_validate(it, from_attributes=True) for it in items
#     ]
#     return sale_out


# @router.get("/sales/{sale_id}/print")
# def print_sale_bill(
#         sale_id: int,
#         db: Session = Depends(get_db),
#         user: User = Depends(auth_current_user),
# ):
#     """
#     Generate a Pharmacy Bill PDF for a sale.
#     """
#     if not has_perm(user, "pharmacy.view"):
#         raise HTTPException(403, "Not permitted")
...
#     patient = db.query(Patient).get(sale.patient_id)
#     if not patient:
#         raise HTTPException(404, "Patient not found")

#     location = db.query(PharmacyLocation).get(sale.location_id)

#     rows = (db.query(PharmacySaleItem, PharmacyMedicine).join(
#         PharmacyMedicine,
#         PharmacyMedicine.id == PharmacySaleItem.medicine_id).filter(
#             PharmacySaleItem.sale_id == sale_id).all())
...
#     pdf_buf = build_sale_bill_pdf(sale, patient, location, item_rows)
#     return StreamingResponse(
#         pdf_buf,
#         media_type="application/pdf",
#         headers={
#             "Content-Disposition":
#             f'inline; filename="PharmacyBill_{sale_id}.pdf"'
#         },
#     )
