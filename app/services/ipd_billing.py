...
def _resolve_rate(db: Session, room_type: str, for_date: date) -> float | None:
    r = (
        db.query(IpdBedRate).filter(
            ...
            IpdBedRate.room_type == room_type,
            IpdBedRate.effective_from <= for_date,
        ).filter((IpdBedRate.effective_to == None)
...
def _add_unbilled_lis_ris_for_admission(
    db: Session,
    inv: Invoice,
    admission_id: int,
    patient_id: int,
    ...
    lis_items = (db.query(LisOrderItem).join(
        LisOrder, LisOrderItem.order_id == LisOrder.id).filter(
            ...
            LisOrder.context_type == "ipd",
            LisOrder.context_id == admission_id,
            LisOrderItem.status.in_(["validated", "reported"]),
        ).all())
    for it in lis_items:
        # price from LabTest
    ...
    ris_orders = (db.query(RisOrder).filter(
        ...
        RisOrder.context_type == "ipd",
        RisOrder.context_id == admission_id,
        RisOrder.status.in_(["reported", "approved"]),
    ).all())
...
def auto_finalize_ipd_on_discharge(
    db: Session,
    *,
    admission_id: int,
    user_id: int | None = None,
    ...
    Behaviour change (for your requirement):
      - Even when this runs, invoice status remains 'draft' by default.
      - Billing team can review and explicitly finalize from UI/API.
    """
    ...
    # Recompute totals but keep status as 'draft' so billing team can review
    if inv.status == "draft":
        _recompute(inv)
        db.flush()
